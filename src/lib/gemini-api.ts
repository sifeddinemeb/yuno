    ...
    try {
      for (let i = 0; i < count; i++) {
        ...
        if (this.mockData) {
          ...
          console.log('Generated mock content:', generatedContent.title);
        } else {
          // Use real Gemini API
...
Adapt the content structure based on the challenge requirements.`;
  ...
  private async callGeminiAPI(prompt: string): Promise<any> {
    ...
    try {
      ...
      console.log('Calling Gemini API at URL:', url.substring(0, url.indexOf('?')));
    ...
    try {
      ...
      try {
        ...
        if (jsonContent && jsonContent[1]) {
          parsed = JSON.parse(jsonContent[1].trim());
        } else {
      ...
      console.log('Successfully parsed response into challenge content');
      return result;
    } catch (error) {
      console.error('Failed to parse Gemini response:', error);
      console.error('Raw response:', response);
      throw new Error(`Invalid JSON response from Gemini API: ${error.message || 'Unknown error'}`);
    }
  }
  ...
  private async generateMockContent(request: ContentGenerationRequest): Promise<GeneratedContent> {
    console.log('Generating mock content for:', request.challengeType);
