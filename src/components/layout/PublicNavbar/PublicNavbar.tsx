...
const PublicNavbar: React.FC = () => {
  ...
  return (
    <>
      <nav className="fixed top-0 left-0 right-0 z-50 backdrop-blur-md bg-white/70 dark:bg-black/40 border-b border-white/10 dark:border-white/5">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          ...
          <button
            ...
            className="md:hidden p-2 rounded-lg hover:bg-black/10 dark:hover:bg-white/10 focus:outline-none"
      ...
      {isOpen && (
        <div className="fixed inset-0 z-40 md:hidden" aria-modal="true" role="dialog">
          <div
            ...
            onClick={closeMobile}
          />
          <div className="absolute top-16 left-0 w-full bg-dark-900 glass-dark p-6 space-y-6 border-t border-white/10 dark:border-white/5">
            {navItems.map(({ label, path }) => (
              <NavLink
                key={path}
                to={path}
