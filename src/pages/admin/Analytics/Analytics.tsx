...
const Analytics = () => {
  ...
  const loadAnalyticsData = async () => {
    try {
      ...
      const [challengeData, dailyData, responsesData, metricsData] = await Promise.all([
        analyticsApi.getChallengePerformance(),
        analyticsApi.getDailyMetrics(),
        responseApi.getForAnalytics(1000),
        analyticsApi.getDashboardMetrics()
      ]);

      setChallengePerformance(challengeData || []);
  ...
  // Mock data for charts when no real data exists
  const responseTimeDistribution = [
    { range: '0-5s', count: rawResponses.filter(r => r.response_time_ms <= 5000).length || 12 },
    { range: '5-10s', count: rawResponses.filter(r => r.response_time_ms > 5000 && r.response_time_ms <= 10000).length || 25 },
    { range: '10-15s', count: rawResponses.filter(r => r.response_time_ms > 10000 && r.response_time_ms <= 15000).length || 8 },
    { range: '15-20s', count: rawResponses.filter(r => r.response_time_ms > 15000 && r.response_time_ms <= 20000).length || 3 },
    { range: '20s+', count: rawResponses.filter(r => r.response_time_ms > 20000).length || 1 },
  ];

  const botActivityData = [
    { type: 'Pattern Recognition Failure', count: 15 },
    { type: 'Response Time Anomaly', count: 8 },
    { type: 'Behavioral Inconsistency', count: 6 },
    { type: 'Multiple Rapid Attempts', count: 4 },
    { type: 'Invalid Input Format', count: 2 },
  ];
    ...
    <div className="space-y-8">
      ...
      >
        ...
        <Card variant="glass" className="text-center">
          ...
          <div className="text-2xl font-bold mb-1">{dashboardMetrics.totalAttempts}</div>
              ...
              <tbody>
                {paginatedData.map((row, index) => (
                  <tr key={index} className="border-b border-gray-800 hover:bg-glass-light transition-colors">
                    <td className="py-3 px-4 font-mono text-sm">{row.session_id.substring(0, 8)}...</td>
                    <td className="py-3 px-4 font-mono text-sm">
                      {row.challenge_id ? `${row.challenge_id.substring(0, 8)}...` : 'N/A'}
                    </td>
